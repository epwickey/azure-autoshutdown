<#
    ...
    .DESCRIPTION
        ...
        There is an additional improvement on this 
...
try
{
    ...
    Write-Output "Current GMT time [$($tcurrentTime.ToString("dddd, yyyy MMM dd HH:mm:ss"))] will be checked against schedules"
    ...
    foreach($vm in $resourceManagerVMList)
    {
		...
		{
            # Schedule not matched. Start VM if stopped only if enddate is greater than current date
			if ( $enddate -gt $(get-date) )
			{
		    #write-output "$enddate $(get-date)"
            Write-Output "[$($vm.Name)]: Current time falls outside of all scheduled shutdown ranges. Starting VM"
            ...
            {
                Write-Output " End date is $enddate. Not starting VM"
            }
		}	 
