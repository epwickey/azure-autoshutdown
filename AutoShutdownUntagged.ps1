...
{	
	...
	$currentTime = [System.TimeZoneInfo]::ConvertTimeFromUtc($tDate, $tz)
	...
	try
	{
	    ...
	    else
	    {
	        ...
	        else
	        {
				...
				if ($(Get-Date  $TimeRange) -lt $(Get-Date) )
	            {
                    #Current date is after the specific date. The server should be shutdown. 
...
function AssertResourceManagerVirtualMachinePowerState
{
    ...
    #write-output "[Resource groupname= $($VirtualMachine.ResourceGroupName)]"
...
try
{
    ...
    Write-Output "Runbook started. Version: $VERSION UTC $currentTime  GMT $tCurrentTime"
    if($Simulate)
    {
    ...
    Write-Output "Current BST time [$($tcurrentTime.ToString("dddd, yyyy MMM dd HH:mm:ss"))] will be checked against schedules"
	...
	$taggedResourceGroups = @(Get-AzureRMResourceGroup -Tag @{'AutoShutdownSchedule'=''})
    $taggedResourceGroupNames = @((Get-AzureRMResourceGroup -Tag @{'AutoShutdownSchedule'=''}).ResourceGroupName)
    Write-Output "Found [$($taggedResourceGroups.Count)] schedule-tagged resource groups in subscription"	

    # Get resource groups with EndDate 
    $endDateResourceGroups = @(Get-AzureRMResourceGroup -Tag @{'EndDate'=''})
    $endDateResourceGroupNames = @((Get-AzureRMResourceGroup -Tag @{'EndDate'=''}).ResourceGroupName)
    Write-Output "Found [$($endDateResourceGroupNames.Count)] enddate-tagged resource groups in subscription"

    # Get resource groups that are tagged for never shutdown of resources
	$taggedResourceGroupsNoShut = @(Get-AzureRMResourceGroup -Tag @{"noshutdown"='yes'})
    $taggedResourceGroupNamesNoShut = @($taggedResourceGroupsNoShut | select -ExpandProperty ResourceGroupName)
    Write-Output "Found [$($taggedResourceGroupsNoShut.Count)] Noshutdown resource groups in subscription"
    ...
    foreach($vm in $resourceManagerVMList)
    {
        ...
        $schedule = $null
        if ( $vm.ResourceType -eq "Microsoft.Compute/virtualMachines" -and $vm.Tags -and $vm.Tags.keys -contains "EndDate")
		{
			$enddate = get-date ($vm.Tags.EndDate) # Check if individual VM has an Enddate tag
		}
		elseif($endDateResourceGroupNames -contains $vm.ResourceGroupName)
                {
                    # Check if Resource Group has enddate tag
                    $parentGroup =  @(get-azurermresourcegroup -name $vm.ResourceGroupName)
                    $endDate = get-date ($parentGroup.Tags.EndDate)
                    Write-Output "Setting end date from RG $endDate"
                }
        else
		{
			$enddate = (get-date).AddDays(1) #If no enddate define enddate to be next day
		}
      if ($vm.ResourceType -eq "Microsoft.Compute/virtualMachines" -and $vm.Tags -and $vm.Tags.Keys -contains "noshutdown" -and $endDate -gt $(get-date) )  
        { #Skip VM's tagged noshutdown. Modified by Eranga
         $NSValue = ($vm.Tags.noshutdown)
         
         if ($NSValue.ToLower() -eq "yes" )
          { 
		   Write-Output "[$($vm.Name)] has  noshutdown tag. Skipping vm "
           write-Output "Enddate is $endDate"
        ...
        if($vm.ResourceType -eq "Microsoft.Compute/virtualMachines" -and $vm.Tags -and $vm.Tags.Keys -eq "AutoShutdownSchedule")
        {
            Write-Output "[$($vm.Name)]: Found AutoShutdownSchedule Tag. Skipping ]"
			continue #ignore the autoshutdownschedule tag
        ...
        {
			Write-Output "[$($vm.Name)]: Found parent resource group schedule tag. Skipping]"
            continue #ignore the autoshutdownschedule tag
        ...
        elseif ( $taggedResourceGroupNamesNoShut -contains $vm.ResourceGroupName -and $endDate -gt $(get-date) )
		{   #vm belongs to noshutdown tagged Resource Group. Skipping. Modified by Eranga
			Write-Output "[$($vm.Name)] is in noshutdown RG. Skipping "
            write-Output "enddate is $endDate"
			continue
		}
		else
        {
            ...
            Write-Output "[$($vm.Name)]: Not tagged for shutdown directly or via membership in a tagged resource group. adding 1900 -> 0700 as temporary schedule to shutdown."
        ...
        if($schedule -eq $null)
        {
            Write-Output "[$($vm.Name)]: Failed to get tagged schedule for virtual machine. Adding 1900-0700 as temp schedule to shutdown the vm."
            continue
        }
		...
		if($scheduleMatched)
		{
		    ...
		    Write-Output "[$($vm.Name)]: Current time [$tcurrentTime] falls within the scheduled shutdown range [$matchedSchedule]"
